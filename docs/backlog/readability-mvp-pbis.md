...
- PBI-05Pは`@textlint/markdown-to-ast@15.8.0`の全Paragraphと`textlint-util-to-string@3.3.4`の
  ...
  acceptanceは13件以上を収集し、AST構造・可視text・RNG-001・blank/raw/document/splitASTの誤実装を表す
  13 titleを必須にする。F04はtitleだけでなくprojected textへの`split`が2文、Paragraphへの`splitAST`が1文になる
  substantive assertionを要求し、placeholder化を拒否する。
