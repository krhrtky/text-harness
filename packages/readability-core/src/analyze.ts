...
import { analyzeD003 } from "./rules/D003.ts";
...
import { analyzeD008 } from "./rules/D008.ts";
...
export function analyze(input: unknown, config: ReadabilityConfig | ValidatedReadabilityConfig | unknown): readonly Finding[] {
  ...
  const findings = Object.values(validated.rules).flatMap((rule): readonly Finding[] => {
    ...
    switch (rule.ruleId) {
      ...
      case "D003": return analyzeD003(input, rule.pairs ?? [], rule.severity);
      ...
      case "D008": return analyzeD008(input, rule.replacements, rule.severity);
